...
export default function Analysis() {
  ...
  const handleSearchButton = () => {
    ...
    if (subDistrict !== 0) {
      ...
      cropQueries = [
        `data=rice_2023${month}${lastCropDate} as b&select=json_agg(features) AS features FROM (SELECT json_build_object('type', 'Feature', 'geometry', ST_AsGeoJSON(b.geom)::jsonb, 'properties', json_build_object('legend', legend, 'rai', rai, 'office_cod', office_cod, 'p_name', p_name, 'a_name', a_name, 't_name', t_name, 'start_date', start_date, 'crop_type', crop_type, 'proj_name', proj_name)) AS features&where=data_date = '${lastDateCrop}' AND t_code='${subDistrict}') AS subquery`,
        `data=maize_2023${month}${lastCropDate} as b&select=json_agg(features) AS features FROM (SELECT json_build_object('type', 'Feature', 'geometry', ST_AsGeoJSON(b.geom)::jsonb, 'properties', json_build_object('legend', legend, 'rai', rai, 'office_cod', office_cod, 'p_name', p_name, 'a_name', a_name, 't_name', t_name, 'start_date', start_date, 'crop_type', crop_type, 'proj_name', proj_name)) AS features&where=data_date = '${lastDateCrop}' AND t_code='${subDistrict}') AS subquery`,
        `data=sugarcane_2023${month}${lastCropDate} as b&select=json_agg(features) AS features FROM (SELECT json_build_object('type', 'Feature', 'geometry', ST_AsGeoJSON(b.geom)::jsonb, 'properties', json_build_object('legend', legend, 'rai', rai, 'office_cod', office_cod, 'p_name', p_name, 'a_name', a_name, 't_name', t_name, 'start_date', start_date, 'crop_type', crop_type, 'proj_name', proj_name)) AS features&where=data_date = '${lastDateCrop}' AND t_code='${subDistrict}') AS subquery`,
        `data=cassava_2023${month}${lastCropDate} as b&select=json_agg(features) AS features FROM (SELECT json_build_object('type', 'Feature', 'geometry', ST_AsGeoJSON(b.geom)::jsonb, 'properties', json_build_object('legend', legend, 'rai', rai, 'office_cod', office_cod, 'p_name', p_name, 'a_name', a_name, 't_name', t_name, 'start_date', start_date, 'crop_type', crop_type, 'proj_name', proj_name)) AS features&where=data_date = '${lastDateCrop}' AND t_code='${subDistrict}') AS subquery`,
      ];
    } else if (district !== 0) {
  ...
  const handleDraw = (geom) => {
    ...
    cropNames.forEach((cropName) => {
      const query = `data=${cropName}_2023${month}${lastCropDate} as b&select=json_agg(features) AS features FROM (SELECT json_build_object('type', 'Feature', 'geometry', ST_AsGeoJSON(b.geom)::jsonb, 'properties', json_build_object('legend', legend, 'rai', rai, 'office_cod', office_cod, 'p_name', p_name, 'a_name', a_name, 't_name', t_name, 'start_date', start_date, 'crop_type', crop_type, 'proj_name', proj_name)) AS features&where=data_date = '${lastDateCrop}' AND ST_Intersects(ST_SetSRID(ST_GeomFromGeoJSON('${JSON.stringify(
        geom
